        ...
        <div class="panel panel-default thumbnail">
            ...
            <div class="panel-heading1">
                <div class="btn-group"  style="margin-left:30px; margin-top:10px;">
                    <a class="btn btn-success" href="<?php echo base_url("client/wholesaler/wholesaler_form") ?>"> <i class="fa fa-plus"></i>  <?php echo "Add Wholesaler" ?> </a>  
                </div>
            </div>

            <div class="panel-body">
                <?php echo form_open('client/wholesaler/wholesaler_list') ?>
                <div class="row">
                    <div class="col-12 col-md-12 " align="center">
                        <div class="col-4 col-md-4">
                            <p><input id="wholesaler_name" type="text" style="min-width: 110px;" class="form-control" name="wholesaler_name"
                                      placeholder="Wholesaler Name"
                                      value="<?php if ($this->session->userdata('search_wholesaler') != null) {
                                          echo $this->session->userdata('search_wholesaler')['wholesaler_name'];
                                      } ?>"></p>                            
                ...
                <table class="display table table-hover table-condensed">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo "Wholesaler Name" ?></th>
                    ...
                    <tbody>
                        <?php if (!empty($wholesalers)) { ?>
                            <?php $sl = 1; ?>
                            <?php foreach ($wholesalers as $whole) { 
							$comp_datail = $this->db->select('comp_name')->from('companies')->where('comp_id', $whole->wholesaler_company_id)->limit(1)->get()->row();
							$class = $this->db->select('cl_name')->from('classes')->where('cl_id', $whole->wholesaler_class)->order_by('cl_id', "desc")->limit(1)->get()->row();
							$segment = $this->db->select('seg_name')->from('segment')->where('seg_id', $whole->wholesaler_segment)->order_by('seg_id', "desc")->limit(1)->get()->row();
							$workinghours = $this->db->select('wh_name')->from('working_hours')->where('wh_id', $whole->wholesaler_working_hours)->order_by('wh_id', "desc")->limit(1)->get()->row();?>
                                <tr class="<?php echo ($sl & 1)?"odd gradeX":"even gradeC" ?>" id="<?php echo $whole->id; ?>">
                                    <td><?php echo $sl; ?></td>
                                    <td><?php echo $comp_datail->comp_name; ?></td>
                                    <td><?php echo $whole->wholesaler_name; ?></td>
                                    <td><?php echo $whole->wholesaler_contact_person; ?></td>
                                    <td><?php echo $workinghours->wh_name; ?></td>
                                    <td><?php echo $class->cl_name; ?></td> 
                                    <td><?php echo $segment->seg_name; ?></td> 
                                    <td class="center" >                                         
                                        <a href="<?php echo base_url("client/wholesaler/wholesaler_form/$whole->id") ?>" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a> 
										<a class="del btn btn-xs btn-danger" data-target="#cmpltadminModal-2" id="<?php echo $whole->id; ?>" data-toggle="modal" href="javascript:void()"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                    </td>
                                </tr>
                                <?php $sl++; ?>
                            <?php } ?> 
...
function removeEntity(id){
	...
	if(id!=""){
		$.ajax({ 
		url: "<?php echo base_url('client/wholesaler/delete');?>",
		method:"POST",
...
$(document).ready(function() {
  ...
  $("body").on("click", ".reset", function() {
	   $('#wholesaler_name').val('');
	   <?php $this->session->unset_userdata('search_wholesaler');?>
	  window.location='<?php base_url('client/wholesaler/wholesaler_list')?>';
  });
});
</script>
